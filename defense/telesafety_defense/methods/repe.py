...
class RePEDefender(InputDefender):
    ...
    def calc_significance(self) -> Tuple[List[Any], List[Any]]:
        ...
        for layer in hidden_layers:
            ...
            sign_values = self.rep_reader.direction_signs[layer]
            if isinstance(sign_values, torch.Tensor):
                sign_value = sign_values.flatten()[0].item()
            elif isinstance(sign_values, (np.ndarray, list, tuple)):
                sign_value = float(np.asarray(sign_values).flatten()[0])
            else:
                sign_value = sign_values

            eval_func = min if sign_value == -1 else max
